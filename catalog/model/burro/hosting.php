...
class ModelBurroHosting extends Model {
	...
	*/  
	...
	public function getHostingsFromSessions($product_id=0) {
		...
		if ( isset( $this->session->data['hostings'] ) ) {
	...
	public function saveHostings($order_id) {    
		...
		//$this->log->write("############# saveHostings dopo che ho importato in config BURRO_SERVERS = ");
		...
		if ( isset( $this->session->data['hostings'] ) ) foreach ( $this->session->data['hostings'] as $hosting ) {
			...
			if ( isset( $hosting['hosting_renew_order_hosting_id'] ) && (int)$hosting['hosting_renew_order_hosting_id'] > 0 ) {
				...
				$hosting_renew_order_hosting_id = (int)$hosting['hosting_renew_order_hosting_id'];
				$date_start = "'".$hosting['hosting_renew_date_start']."'"; 
				$date_end = "'".$hosting['hosting_renew_date_end']."'"; 
				$server = $hosting['hosting_renew_server']; 
				//$registrant = $this->db->escape( serialize( json_decode( rawurldecode( $hosting["hosting_registrant"] ) ) ) );
				$registrant = rawurldecode( $hosting["hosting_registrant"] );
			} else {
				//sto salvando un nuovo hosting
				$state = "new"; //l'hosting nuovo deve essere "new"
			...
			$this->db->query("INSERT INTO `".DB_PREFIX."order_hosting` SET order_id = '" . $order_id . "', order_product_id = '" . $hosting["product_id"] . "', date_start = ".$date_start.", date_end = ".$date_end.", state = '".$state."', renew_order_hosting_id = '".$hosting_renew_order_hosting_id."', server = '".$server."', domain = '".$hosting["hosting_domain_selected"]."', email = '".$hosting["hosting_mailbox_selected"]."', registrant = '".$registrant."' ");
			$order_hosting_id = mysql_insert_id();
			//se sto rinnovando devo mettere il vecchio service a "renewed"
			if ( $hosting_renew_order_hosting_id > 0 ) {
				$result = $this->db->query( "UPDATE  ".DB_PREFIX."order_hosting SET state = 'renewed' WHERE order_hosting_id = ".$hosting_renew_order_hosting_id );
			}
			
			//also save tasks
			if ( isset( $hosting['hosting_renew_order_hosting_id'] ) && (int)$hosting['hosting_renew_order_hosting_id'] > 0 ) {
				//sto facendo un rinnovo
				//quindi non genero nessun task da fare perchè si pruppone che il servizio già esista e non vada creato
			} else {
				//sto salvando un nuovo hosting
				$this->load->model('burro/task');
				//ciclo sui singoli services di cui è composto l'hosting
				foreach ( $hosting['hostings'] as $service ) {
					//creo un task per ogni service, passando sempre anche l'oggetto hosting che contiene i parametri scelti dall'utente
					$this->model_burro_task->newTask($service["service"],array($service,$hosting),$order_hosting_id); 
				} 
			}
			
			//infine resetto le sessions per azzerare il cart anche per gli hostings
			$this->resetHostingSessions(); 
		}
	}
	
	public function resetHostingSessions() {
	...
	public function getMailboxes($customer_id=0) {
		require("admin/config_burro.php"); 
		//to get mailboxes must do 2 queries directly on the ispconfig database (there are no remote API methods to do this)
		//first query is to get the ispconfig client id from the opencart username (they are the same)
			//se se sto inserendo utenti pregressi e sto usando la mia mail invece di quella del cliente finale?
		//second query is to get mailboxes from client id
		
		//get access from config of default server
		foreach ( $BURRO_SERVERS as $server_cfg) {
			if ( $server_cfg["name"] == $BURRO_DEFAULT_SERVER_NAME ) {
				$ispconfig_data = $server_cfg;
				break;
			}
		}
		
		//let's get ispconfig sys_userid
		$sys_userid = 0;
		...
		if ($mysqli->connect_errno) {
			$this->log->write("Connect failed: %s\n", $mysqli->connect_error);
			exit();
		}
		//username saved in ispconfig is equal to the username of opencart, minus the "@" char
		//$this->log->writeVar($this->customer->getEmail());
		if ($this->customer->getEmail() != "" && $result = $mysqli->query("SELECT userid FROM sys_user WHERE username = '".str_replace("@","",$this->customer->getEmail())."';") ) {
			if ($result->num_rows > 0) while($row = $result->fetch_array(MYSQLI_ASSOC)) {
				$sys_userid = $row["userid"];
			}
			$result->close();
		}
		
		//and with sys_userid get mailboxes
		$mailboxes = array();
		if ($sys_userid > 0 && $result = $mysqli->query("SELECT email FROM mail_user WHERE sys_userid = '".$sys_userid."';") ) {
			// generazione di un array numerico
			if ($result->num_rows > 0) while($row = $result->fetch_array(MYSQLI_ASSOC)) {
				array_push( $mailboxes, array("email"=>$row["email"])  ); 
			}
			$result->close();
		}
